      ...
      <section className="relative overflow-hidden py-24 px-4">
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          ...
          <h1 className="text-4xl md:text-6xl font-bold text-foreground leading-tight mb-6">
            {t(locale, 'hero.title_line1')}<br />
            <span className="gradient-text">{t(locale, 'hero.title_line2')}</span>
          </h1>
          <p className="text-xl text-muted max-w-2xl mx-auto mb-8">
            {t(locale, 'hero.subtitle')}
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href="/blog" className="btn-secondary text-center">{t(locale, 'hero.cta_articles')}</Link>
            <Link href="/fr/pricing" className="btn-primary text-center">
              {t(locale, 'hero.cta_start')}
            </Link>
          </div>
        </div>
      </section>
        ...
        {recent.length > 0 && (
          <section className="mb-16">
            ...
            <div className="text-center mt-8">
              <Link href="/blog" className="btn-secondary">{t(locale, 'home.view_all')}</Link>
            </div>
          </section>
        )}
      </div>
