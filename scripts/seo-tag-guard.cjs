...
const RULES = [
  ...
  {
    code: "no_optin_ga4_consent",
    severity: "critical",
    mode: "forbid",
    pattern: /analytics_storage\s*:\s*['"`]denied['"`]/,
    msg: "BLOQUÉ : analytics_storage='denied' par défaut détecté — bloque GA4 pour tous les visiteurs. Pattern interdit.",
    fix: "Utiliser le pattern opt-out VERROUILLÉ : `rejected ? 'denied' : 'granted'` (granted par défaut).",
  },
  {
    ...
    msg: "BLOQUÉ : ID GA4 banni détecté (G-D3XG48MBSM ou G-7HQQDGHRT2) — propriétés archivées.",
  ...
  },
  {
    ...
    msg: "BLOQUÉ : pattern consent opt-out absent — `rejected ? 'denied' : 'granted'` obligatoire.",
    fix: "Ajouter le pattern verrouillé dans le script consent inline.",
  },
  {
